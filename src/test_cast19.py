...
from data_structure import ConvDataset, ConvDataset_cast, ConvDataset_rewrite
import os
from os import path
...
def get_test_query_embedding(args):
    ...
    '''
    test_dataset = ConvDataset_cast(args, query_tokenizer, passage_tokenizer, args.test_file_path, add_doc_info=False)
    test_loader = DataLoader(test_dataset, 
                            batch_size = args.eval_batch_size, 
                            shuffle=False, 
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, disable=args.disable_tqdm):
            ...
            if args.test_type == "rewrite":
                ...
                bt_raw_query = batch['bt_raw_query'].to(args.device)
                bt_raw_query_mask = batch['bt_raw_query_mask'].to(args.device)
                bt_history_context = batch['bt_history_context'].to(args.device)
                bt_history_context_mask = batch['bt_history_context_mask'].to(args.device)
            else:
                raise ValueError("test type:{}, has not been implemented.".format(args.test_type))
            
            if args.test_type == "context_fuse":
...
def output_test_res(query_embedding2id,
    ...
    for record in data:
        ...
        #qid2query[record["id"]] = record["input"][-1]
        #qid2convid[record["id"]] = record["topic_number"]
        #qid2turnid[record["id"]] = record['query_number']
    ...
    #output_file = oj(args.qrel_output_path, "ANCE_qp_soft_res_.json")
    output_trec_file = oj(args.qrel_output_path, "ANCE_t5_oracle_kd_0.5_prefix.trec")
...
# python test_cast19.py --config=Config/test_cast19.toml
